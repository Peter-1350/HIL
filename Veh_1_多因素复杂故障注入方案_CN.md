...
> **修订声明（2026-08-20）**：本文件中的 CF-01～CF-06 不能作为当前阶段的“有效复杂故障”直接执行。尤其是“持续合法油门”没有独立期望值参照时，只是稳态工况，不能被重新定义为油门卡滞故障。本文件保留为既有驾驶员层时序编排记录；后续内部注入端的筛选与复杂故障设计以 [Veh_1_模型内部故障注入候选接口_CN.md](Veh_1_模型内部故障注入候选接口_CN.md) 为准。
